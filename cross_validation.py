...
def cv_loss(y, x, k_indices, method, args, compute_loss):
	...
	for k in range(k_fold):
		...
		if method in [logistic_regression, reg_logistic_regression]:
			# For negative log-likelihood: normalise by sample size to compare train and test losses
			k_loss_tr = k_loss_tr / len(y_tr)
			k_loss_te = neg_log_likelihood(y_te, x_te, weights)
			k_loss_te = k_loss_te / len(y_te)
		else:
...
def cv_criterions(y, x, k_indices, method, args, verbose=True):
	...
	for k in range(k_fold):
		...
		accuracy, precision, recall, f1_score = compute_criterions(y_te, x_te, weights, method)
	...
	# average the k results
	cv_accuracy = np.sum(accuracy_te) / k_fold
	cv_precision = np.sum(precision_te) / k_fold
	cv_recall = np.sum(recall_te) / k_fold
	cv_f1_score = np.sum(f1_score_te) / k_fold
	...
	return accuracy_te, precision_te, recall_te, f1_score_te
